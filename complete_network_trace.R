...
# This version support the 4 heuristics to test
complete_trace<-function(g1,g2,taxnames='default',node1="default", node2="default",maxdistance=0, maxtime=3600,maxnode=0, heuristic=1) 
{
...
if (length(g2_unique_names_primed)==0) {
	...
	j=node2_number;
			...
			if (i>j||(node1_number!=0&&i!=j)) {
				g2_unique_k=as.numeric(V(g2_without_k)[g2_unique_names_primed]);
				...
				if (heuristic==1||heuristic==4) {
					...
					if(g1short_ij>iso_g3short_ij ){
						...
						# il se peut que l'on ne puisse pas passe par k car le chemin passe par a et b
							...
							if (found) {
								...
								if (iso_g3short_ij>g1short_ij) {
									...
									trace_path_type=c_equal;
									 ...
									 if (trace) {
										...
										if (length(paths)>0) {											
											# Note: for now, we only get the first paths
											# We must be carefull here
											trace_path=c();
											for (p in paths) 
												if (good_path(p,g2_unique_k)&&length(trace_path)==0) 
													for (k in p) {    
													trace_path=c(trace_path,V(g2_without_k)[k]$name);
													trace_type=c(trace_type,V(g2_without_k)[k]$tax);
												 }										 
								...
								deadend=TRUE;					
								#current solution
								c_trace_len=9999999; #solution len
								c_trace=c();   #solution trace
								c_trace_path=c();
								c_trace_type=c();																
								c_found=FALSE; #solution found			
								...
								if (length(list_of_knames)>0)
									...
									if (deadend||heuristic==4) {
										...
										if (length(paths1)>0&&length(paths2)>0) {
											...
											#
											intersect=TRUE;
											# Note, we take the first one here, which could end up in a longer solution
											# 
											for (p in paths1) { 										
												if (intersect||heuristic==4) {												
													...
													for (q in paths2) {
														...
														if(any(V(g2_without_k_and_j)[p] %in% V(g2_without_k_and_i)[q])) {
															...
															if (length(paths3)>0) {
																...
																c_found=TRUE;
																if (trace_len<c_trace_len) {
																	c_trace_len=trace_len;
																	c_trace_path=trace_path;
																	c_trace_type=trace_type;
																}
																intersect=FALSE;
															}													
														} else {														
								...
								if (!deadend) {
									trace_path_type=c_detour;
									detour=detour+1;
									#We return the best path here
									trace_len=c_trace_len;
									trace_path=c_trace_path;
									trace_type=c_trace_type;
					...
					if (heuristic==2) {
						path=heuristic2(g2_without_k,g2_unique_k, g1names[i], g1names[j]); 
					} else  if (heuristic==3) {
						path=heuristic3(g2_without_k,g2_unique_k, g1names[i], g1names[j]); 
					} #Added here
					...
					#Logic here
					#print(path);
					d1=as.numeric(g1short_ij);
					d2=as.numeric(path$dist);
					trace_len=path$dist;
					trace_path=path$path;
					# Need to update the type here
					# Trace taxa
					if (trace_len>0&&is.finite(trace_len))trace_type=V(g2_without_k)[path$path]$tax;
				   ...
				   if (!is.finite(d1)&&!is.finite(d2)){
						trace_path_type=c_inf;
						inf=inf+1;
						if (verbose) cat(g1names[i],g1names[j],"Disconnected nodes",d1,d2,sep="\t", file=filename, append=TRUE);	
				   } else 
				   if (d1==d2&&is.finite(d1)) {
						trace_path_type=c_equal;
						if (verbose) cat(g1names[i],g1names[j],"Equal",d1,d2,sep="\t", file=filename, append=TRUE);	
						egal=egal+1;
				   } else 
				   if (is.finite(d1)&&!is.finite(d2)) {
						trace_path_type=c_deadend;
						if (verbose) cat(g1names[i],g1names[j],"Dead",d1,d2,sep="\t", file=filename, append=TRUE);
						dead=dead+1;
					} else
				   if (d1<d2) {
					if (verbose) cat(g1names[i],g1names[j],"Detour",d1,d2,sep="\t", file=filename, append=TRUE);
					trace_path_type=c_detour;
					detour=detour+1;
				   } else
				   if (d1>d2) {
				   trace_path_type=c_shortcut;
					if (verbose) cat(g1names[i],g1names[j],"Shortcut",d1,d2,sep="\t", file=filename, append=TRUE);
				   rac=rac+1;
				   } else 
				   {
						trace_path_type=c_error;
						if (verbose) cat(g1names[i],g1names[j],"Error",d1,d2,sep="\t", file=filename, append=TRUE);
						error=error+1;
					}
				   }				
				}
